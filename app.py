...
try:
    ...
    st.error("Error: Configura la API KEY en los secretos de Streamlit")

# --- LÓGICA COVENIN ---
def determinar_clase(d):
    if d.get("Aflatoxina") != "Negativo": return "Rechazado"
    # Lógica basada en tu tabla (Maíz Acondicionado)
...
with st.form("registro_maestro"):
    ...
    f_silo = c2.text_input("Silo", value=cabe.get('silo', ''))
    ...
    if st.form_submit_button("✅ REGISTRAR Y GUARDAR"):
        ...
        st.success(f"Registro exitoso. Clasificación automática: {clase}")

# 4. REPORTES
if os.path.exists("aprobados.xlsx"):
    st.divider()
    st.subheader("📊 Reporte Diario (Aprobados)")
    df_ap = pd.read_excel("aprobados.xlsx")
    st.metric("Promedio Humedad", f"{df_ap['Humedad'].mean():.2f}%")
    st.dataframe(df_ap)
